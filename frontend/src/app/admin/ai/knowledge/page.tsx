...
function KnowledgeHeader({ view, selectedKb, syncing, t, onBack, onSyncAll, onCreate }: {
  view: 'list' | 'detail';
  selectedKb: KnowledgeBaseOut | null;
  syncing: boolean;
  t: (k: string) => string;
  onBack: () => void;
  onSyncAll: () => void;
  ...
  return (
    <div className="flex items-center justify-between mb-6">
      <div className="flex items-center gap-3">
        {view === 'detail' && (
          <button onClick={onBack} className="p-2 text-gray-400 hover:text-black rounded-lg hover:bg-gray-100 transition-colors">
      ...
      {view === 'list' && (
        <div className="flex items-center gap-3">
          ...
          <button onClick={onCreate} className="flex items-center gap-2 px-4 py-2 text-sm text-white bg-black rounded-lg hover:bg-gray-800 transition-colors">
...
function RagConfigBar({ ragConfig, t, onToggle, onUpdateParam, onToggleRerank }: {
  ragConfig: RagConfigOut;
  t: (k: string) => string;
  onToggle: () => void;
  onUpdateParam: (key: 'top_k' | 'threshold' | 'max_threshold', value: number) => void;
  onToggleRerank: () => void;
}) {
  return (
          ...
          <>
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                onChange={e => onUpdateParam('top_k', Number(e.target.value))}
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="relative w-32 h-5">
                ...
                <input type="range" min="0" max="1" step="0.01" value={ragConfig.threshold} onChange={e=>{const v=+e.target.value;if(v<=(ragConfig.max_threshold??.7))onUpdateParam('threshold',v)}} className="dr" />
                <input type="range" min="0" max="1" step="0.01" value={ragConfig.max_threshold??.7} onChange={e=>{const v=+e.target.value;if(v>=ragConfig.threshold)onUpdateParam('max_threshold',v)}} className="dr" />
              </div>
...
function CreateKbModal({ createName, setCreateName, createDesc, setCreateDesc, creating, t, onCreate, onClose }: {
  createName: string;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex justify-end gap-3 mt-6">
          <button onClick={onClose} className="px-4 py-2 text-sm text-gray-600 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">{t('common.cancel')}</button>
          <button onClick={onCreate} disabled={creating || !createName.trim()} className="px-4 py-2 text-sm text-white bg-black rounded-lg hover:bg-gray-800 disabled:opacity-50 transition-colors">
...
function KbList({ kbs, t, ragConfig, onEnter, onDelete, onToggleRagKb }: {
  kbs: KnowledgeBaseOut[];
  t: (k: string) => string;
  ragConfig: RagConfigOut | null;
  onEnter: (kb: KnowledgeBaseOut) => void;
  onDelete: (kb: KnowledgeBaseOut) => void;
  onToggleRagKb: (kbId: number) => void;
}) {
  return (
        ...
        {kbs.length === 0 ? (
          ...
          kbs.map(kb => (
            ...
            >
              ...
              {ragConfig && (
                <button
                  onClick={e => { e.stopPropagation(); onToggleRagKb(kb.id); }}
...
function UploadArea({ fileInputRef, uploadFile, uploading, setUploadFile, t, onUpload }: {
...
function KbConfigEdit({ kbConfig, setKbConfig, savingConfig, embeddingProvider, embeddingModel, rerankerProvider, rerankerModelGlobal, t, onSave }: {
  kbConfig: Partial<KnowledgeBaseOut>;
  setKbConfig: (fn: (p: Partial<KnowledgeBaseOut>) => Partial<KnowledgeBaseOut>) => void;
  savingConfig: boolean;
  embeddingProvider: string;
  embeddingModel: string;
  rerankerProvider: string;
  rerankerModelGlobal: string;
  t: (k: string) => string;
  onSave: () => void;
}) {
  return (
...
function KbConfigReadonly({ selectedKb, embeddingProvider, embeddingModel, rerankerProvider, rerankerModelGlobal, t }: {
  selectedKb: KnowledgeBaseOut;
  embeddingProvider: string;
  embeddingModel: string;
  rerankerProvider: string;
  rerankerModelGlobal: string;
...
function KbConfigPanel({ selectedKb, editConfig, kbConfig, savingConfig, embeddingProvider, embeddingModel, rerankerProvider, rerankerModelGlobal, t, onCancelEdit, onStartEdit, setKbConfig, onSave }: {
  selectedKb: KnowledgeBaseOut;
  editConfig: boolean;
  kbConfig: Partial<KnowledgeBaseOut>;
  savingConfig: boolean;
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
      <div className="px-5 py-4 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
        ...
        <button
          onClick={editConfig ? onCancelEdit : onStartEdit}
      ...
      <div className="p-5 space-y-5">
        {editConfig ? (
          <KbConfigEdit
            kbConfig={kbConfig}
            setKbConfig={setKbConfig}
            savingConfig={savingConfig}
            embeddingProvider={embeddingProvider}
            embeddingModel={embeddingModel}
            rerankerProvider={rerankerProvider}
            rerankerModelGlobal={rerankerModelGlobal}
            t={t}
            onSave={onSave}
          />
        ) : (
...
function DocumentList({ docs, docLoading, t, onReprocess, onDelete }: {
  docs: KnowledgeDocumentOut[];
  docLoading: boolean;
  t: (k: string) => string;
...
function SearchPanel({ searchQuery, setSearchQuery, searching, searchResults, t, onSearch }: {
  searchQuery: string;
  setSearchQuery: (v: string) => void;
  searching: boolean;
  searchResults: KnowledgeChunkOut[];
  t: (k: string) => string;
  onSearch: () => void;
}) {
  return (
...
export default function AdminKnowledgePage() {
  ...
  const handleSearch = async () => {
    ...
    setSearching(true);
    try {
      const topK = ragConfig?.top_k ?? 10;
      const threshold = ragConfig?.threshold ?? 0.0;
      const maxThreshold = ragConfig?.max_threshold ?? 1.0;
      ...
      {view === 'detail' && selectedKb && (
        <div className="space-y-6">
          ...
          <KbConfigPanel
            ...
            rerankerModelGlobal={rerankerModelGlobal}
            t={t}
            onCancelEdit={handleCancelConfig}
            onStartEdit={enterKbConfig}
            setKbConfig={setKbConfig}
