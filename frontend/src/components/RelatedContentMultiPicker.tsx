...
export default function RelatedContentMultiPicker({ targets, values, onChange }: Props) {
  const [open, setOpen] = useState(false);
  const [tab, setTab] = useState('');
  const [keyword, setKeyword] = useState('');
  const [page, setPage] = useState(1);
  const [data, setData] = useState<{ total: number; pages: number; records: ContentItem[] }>({ total: 0, pages: 0, records: [] });
  const [byType, setByType] = useState<Record<string, Set<number>>>({});
  const [preloaded, setPreloaded] = useState<ContentItem[]>([]);
  const [loading, setLoading] = useState(false);
  const [tabLoading, setTabLoading] = useState(false);

  const types = useMemo(() => Array.from(new Set(targets.map(t => t.type))), [targets]);

  // destination field for each type (one type may map to one field)
  const fieldByType = useMemo(() => {
    const map: Record<string, string> = {};
    targets.forEach(t => { map[t.type] = t.field; });
    return map;
  }, [targets]);

  const labelByType = useMemo(() => {
    const map: Record<string, string> = {};
    targets.forEach(t => { map[t.type] = t.label; });
    return map;
  }, [targets]);

  // merge current selected ids from all target fields
  const selectedIdsForType = useCallback((type: string): number[] => {
    const field = fieldByType[type];
    return values[field] ? values[field].split(',').map(Number).filter(Boolean) : [];
  }, [values, fieldByType]);
  ...
  useEffect(() => {
    ...
    types.forEach(type => initial[type] = new Set(selectedIdsForType(type)));
    setByType(initial);
    setTab(types[0]);
  }, [open, types, selectedIdsForType]);
  ...
  useEffect(() => {
    if (open && tab) loadTab(tab, keyword, page);
  }, [open, tab, keyword, page, loadTab]);
  ...
  useEffect(() => {
    const hasAny = targets.some(t => selectedIdsForType(t.type).length > 0);
    if (!hasAny) { setPreloaded([]); return; }
    const load = async () => {
      setLoading(true);
      const items: ContentItem[] = [];
      for (const t of targets) {
        const ids = selectedIdsForType(t.type);
        if (ids.length === 0) continue;
        try {
          const res = await api.get('/admin/related-content/batch', { params: { type: t.type, ids: ids.join(',') } });
  ...
  const totalSelected = types.reduce((sum, type) => sum + (byType[type]?.size || 0), 0);

  const confirm = () => {
    const next = { ...values };
    for (const t of targets) {
      next[t.field] = Array.from(byType[t.type] || []).join(',');
    }
    onChange(next);
    setOpen(false);
  };

  const hasSelection = targets.some(t => selectedIdsForType(t.type).length > 0);

  return (
    <div>
      <label className="block text-sm font-medium text-gray-700 mb-1">关联内容</label>
      <div
        onClick={() => setOpen(true)}
        className="flex items-center gap-1.5 p-2 border border-gray-300 rounded-md min-h-[38px] cursor-pointer hover:border-blue-400 overflow-x-auto"
      >
        {loading && <span className="text-xs text-gray-400 shrink-0">加载中...</span>}
        {targets.flatMap(t => {
          const items = preloaded
            .filter(i => i.type === t.type && (values[t.field] || '').split(',').map(Number).includes(i.id));
          return items.map(item => (
            <span key={`${t.type}-${item.id}`} className="inline-flex items-center gap-1 px-2 py-0.5 bg-blue-50 text-blue-700 rounded text-xs whitespace-nowrap shrink-0">
              {item.title}
            </span>
          ));
        })}
        {!loading && !hasSelection && <span className="text-xs text-gray-400 shrink-0">未选择</span>}
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={(e) => { if (e.target === e.currentTarget) setOpen(false); }}>
          <div className="bg-white rounded-xl shadow-xl w-[680px] max-h-[80vh] flex flex-col">
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200">
              <h3 className="text-base font-semibold">选择关联内容</h3>
            ...
            <div className="flex gap-1 px-5 pt-3 border-b border-gray-200">
              {types.map(type => (
                <button key={type} onClick={() => setTab(type)} className={`px-3 py-1.5 text-xs rounded-t cursor-pointer ${tab === type ? 'bg-blue-50 text-blue-700 font-medium border border-b-white border-gray-200 -mb-px' : 'text-gray-500 hover:text-gray-700'}`}>
                  {labelByType[type] || type}
                  <span className="ml-1 text-[10px] text-gray-400">{byType[type]?.size || 0}</span>
                </button>
              ))}
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-5 py-2">
              <div className="py-2 text-xs text-gray-400">将归入「{labelByType[tab] || tab}」</div>
              ...
              {!tabLoading && data.records.map(item => (
                <label key={item.id} className="flex items-center gap-3 py-2 px-2 rounded hover:bg-gray-50 cursor-pointer">
                  <input type="checkbox" checked={byType[tab]?.has(item.id)} onChange={() => toggle(item.id)} className="rounded border-gray-300" />
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-medium truncate">{item.title}</div>
                    {item.summary && <div className="text-xs text-gray-400 truncate">{item.summary}</div>}
                  </div>
                  <span className="text-[10px] text-gray-400 shrink-0">{labelByType[item.type] || item.type}</span>
                </label>
              ))}
            </div>
            ...
            <div className="flex items-center justify-between px-5 py-3 border-t border-gray-200 bg-gray-50 rounded-b-xl">
              <span className="text-xs text-gray-500">已选 {totalSelected} 项</span>
